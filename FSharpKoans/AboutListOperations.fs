    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec len lst fin = 
                match lst with 
                |[] -> fin 
                |_::rest -> len rest (fin+1)  
            len xs 0 // write a function to find the length of a list
...
*)
    ...
    let ``17 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec part fn lst1 lst2 lst3 =
                match lst1 with 
                |[] -> lst2,lst3
                |a::rest -> match fn a with 
        ...
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec find fn lst= 
                ...
                |a::rest -> match fn a with 
                            |true -> Some a 
                            |false -> find fn rest
            find p xs // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.tryfind%5b't%5d-function-%5bfsharp%5d
