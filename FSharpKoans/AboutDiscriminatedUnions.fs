...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        let randomOpinion degree =
            ...
            | BSc _ -> "!!SCIENCE!!"
            | BPharm -> "Meh, it's OK."
            | BCom (Economics, Management) | BCom (Management, Economics) | BCom (_, Economics) | BCom (Economics, _) | BCom (_,_) -> "Money, money, money."
            | BA (Philosophy, _) | BA ( _, Philosophy) -> "A thinker, eh?"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("H",Node("e",Node("l",Node("l",Empty,Empty),Empty),Empty),Node("b",Empty,Node("f",Empty,Empty))) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
